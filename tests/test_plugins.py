...
def test_run_image_detector_plugins_after(plugs):
    run_image_detector_plugins_after(ns['loaded_plugins'], True, 0, .5, load_numpy_img('./resources/test_img_labels.jpg'))
...
def test_run_image_detector_plugins_after_loop(plugs):
    ...
    while(cap.isOpened()):
        ...
        if image_np is None:
            print('Image is none at ix {}'.format(ix))
            break
        ix += 1
        run_image_detector_plugins_after(ns['loaded_plugins'], True, ix, .5, image_np)
